...
`
  ...
  /** `abox leave` must be able to end the shell, so it is a shell function. */
...
export PS1='\\[\\033[36m\\]abox:${space.name}\\[\\033[0m\\] \\W $ '
...
abox() {
  case "$1" in
    ...
    status)
      ...
      ;;
    *)
      echo "abox: inside a space you can use: leave [--keep], status" >&2
      return 1
      ;;
  esac
}
