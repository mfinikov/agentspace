...
> Installs as `agentpen`; the command is `apen`.

```bash
apen new      # you are now inside a fresh, isolated environment
claude        # or codex, or whatever you run — it lives in here
apen leave    # the environment and everything in it is deleted
```
...
```
  ...
  gone       everything, the moment you run `apen leave`
```
...
`apen new` works out of the box.

```bash
apen doctor    # check this machine can run spaces
```

## Use it

```bash
apen new                  # random name, drops you straight in
apen new scratch          # name it
apen new --net none       # no network at all
apen new --keep           # survives `leave`, re-enter later
apen new --env OPENAI_API_KEY   # forward one host variable in
apen new --stack minimal        # just the rules, none of the scaffolding
apen new --no-start             # scaffold it now, start it later
```

Once inside, every command runs in the space. When you are done:

```bash
apen leave                # destroy it, return to your machine
apen leave --keep         # detach, keep it for `apen enter`
apen status               # what is shared, what is blocked
```
...
|---|---|
| `apen ls` | every space, its state, size and lifetime |
| `apen enter <name>` | go back into a kept space |
| `apen exec -- <cmd>` | run one command inside without attaching |
| `apen status [name]` | isolation settings and disk usage |
| `apen stop [name]` | halt a space, keep its files, release its memory |
| `apen down` | stop every space and the build VM |
| `apen rm <name...>` | destroy specific spaces |
| `apen prune` | destroy every ephemeral space |
| `apen doctor` | check backends, templates and config |
| `apen config` | view or change defaults |

## Ephemeral means ephemeral

`apen leave` deletes the container **and** the workspace. Git commits inside
a space die with it. To keep work, do one of:

- `git push` to a real remote from inside,
- `apen leave --keep` (or create it with `--keep`),
...
deleting anything rather than assuming. Only an explicit `apen leave` destroys
without a prompt. Turn the prompt off with `apen config set confirmOnLeave false`.
...
```bash
apen config                        # show everything
apen config set network none       # default new spaces to offline
apen config set confirmOnLeave false
apen config set forwardEnv ANTHROPIC_API_KEY,GITHUB_TOKEN
```
...
| `confirmOnLeave` | `true` | ask before deleting when a shell ends without `apen leave` |
| `image` | `agentspace/base:0.6` | container image |
...
- **`apen leave --keep` stops the environment**, it does not just detach.
  Keeping your files should not mean keeping a VM resident. `apen enter`
  brings it back with everything intact.

Check what is running at any time:

```bash
apen doctor        # reports guest VMs and their combined memory
apen stop <name>   # halt one space, keep its files
apen down          # stop everything agentpen started
