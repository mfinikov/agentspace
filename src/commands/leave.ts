...
export async function cmdLeave(args: Args): Promise<number> {
  // Inside a space, `aspace leave` is handled by the shell function in the
  ...
  if (!target) {
    throw new UserError(
      'no space to leave',
      'you are not in a space. `aspace ls` shows what exists.',
    )
  }
...
export async function cmdRm(args: Args): Promise<number> {
  ...
  if (!targets.length) throw new UserError('nothing to remove', 'usage: aspace rm <space>')
