...
`
  ...
  isRunning(): boolean {
    // Nothing survives between commands, so a native space is never "running".
    return false
  }
  ...
  /** `apen leave` must be able to end the shell, so it is a shell function. */
...
export PS1='\\[\\033[36m\\]apen:${space.name}\\[\\033[0m\\] \\W $ '
...
apen() {
  case "$1" in
    ...
    status)
      ...
      ;;
    *)
      echo "apen: inside a space you can use: leave [--keep], status" >&2
      return 1
      ;;
  esac
}
