...
aspace status            this space's isolation settings
aspace leave             destroy the space and return to the host
```

Committing is not saving: `aspace leave` deletes the git history with
everything else. Push to a remote, or use `aspace leave --keep`.
